...
util.inherits(DesignDocument, Database);

DesignDocument.prototype.ls = function(opts, cb) {
  opts = this.merge(opts);
  opts.qs = opts.qs || {};
  opts.qs.startkey = utils.stringify(
    keys.design + '/', this.options.stringify);
  ...
  //var q = cdb.util.stringify({
    //startkey: cdb.parameters.design + '/'
  //}, stringify);
