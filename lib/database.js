...
var Database = function(options) {
  ...
  this.att = Attachment(this);
  this.sec = Security(this);
  //console.log('is equal options %s', this.options === );
}
...
Database.prototype.bulk = function(opts, cb) {
  opts = this.merge(opts);
  assert(typeof opts.body === 'string', 'bulk document body must be string');
 ...
 */
Database.prototype.replicate = function(opts, cb) {
  opts = this.merge(opts);
  assert(typeof opts.body === 'string', 'replicate document body must be string');
  var u = this.url([opts.server, opts.db, keys.diff]);
  var req = {
    url: u,
    method: methods.post,
...
Database.prototype.limit = function(opts, cb) {
  ...
  if(limit) req.body = this.stringify(limit);
  return this.request(req, opts, cb);
}
