...
function AbstractCouch(options, server) {
  ...
  if(this.options.basic && this.options.username && this.options.password) {
    this.options.headers = this.options.headers || {}; 
    this.options.headers = {
      'Authorization': 'Basic '
        + new Buffer(this.options.username
            + ':' + this.options.password).toString('base64')};
  }
}
...
function onResponse(err, res, body, cb, req, opts, item) {
  ...
  if(status
    && supportedEvents[code]
    && this.emitter().listeners(code).length) {
    return this.emitter().emit(code, err, res, doc, cb);
  }
...
function getRequest(req, opts) {
  var headers = this.options.headers || {}, z, ct = 'content-type';
  req.qs = {};
  for(z in opts.qs) {
    req.qs[z] = opts.qs[z];
  }
...
function getHeadDocument(err, res, doc, opts) {
  opts = opts || {};
  var status = res && res.statusCode ? res.statusCode: 0;
  var headers = res ? res.headers : null;
  var body = {status: status, headers: headers};
