...
var log = logger({level: logger.NONE});
...
var AbstractCouch = function(options) {
  ...
  // HACK: this references a module level variable
  // when really Document/Attachment etc should be referencing the same *options*
  this.log = log;
}
...
AbstractCouch.prototype.getRequest = function(req, opts) {
  ...
  if(req.body
    ...
    req.body = this.options.stringify(req.body, null, 0);
    console.log('stringify body %s', req.body);
  }
