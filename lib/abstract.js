...
AbstractCouch.prototype.getRequest = function(req, opts) {
  ...
  if(!req.qs.rev) delete req.qs.rev;
...
AbstractCouch.prototype.getHeadDocument = function(err, res, doc, opts) {
  opts = opts || {};
  var doc = {status: res.statusCode, headers: res.headers};
  if(res.statusCode === 200) {
    if(res.headers['content-length']) {
      doc.size = parseInt(res.headers['content-length']);
    }
    if(res.headers.etag) {
      doc.rev = etag(res.headers);
    }
    if(opts.attname) {
      doc.name = opts.attname;
      doc.md5 = res.headers['content-md5'];
      doc.type = res.headers['content-type'];
 ...
 *  Send a PUT request.
 */
AbstractCouch.prototype.put = function(opts, cb) {
  opts = this.merge(opts);
 ...
 *  Send a POST request.
 */
AbstractCouch.prototype.post = function(opts, cb) {
  opts = this.merge(opts);
 ...
 */
AbstractCouch.prototype.del = function(opts, cb) {
  opts = this.merge(opts);
 ...
 *  Send a HEAD request.
 */
AbstractCouch.prototype.head = function(opts, cb) {
  opts = this.merge(opts);
  ...
  return this.request(req, opts, function(err, res, doc) {
    ...
    if(err && !notfound) return cb(err, res);
 ...
 *  Send a COPY request.
 */
AbstractCouch.prototype.copy = function(opts, cb) {
  opts = this.merge(opts);
