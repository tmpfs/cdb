...
var AbstractCouch = function(options, server) {
  ...
  this.jar.user = this.jar.user || {};
  this.jar.cookie = this.jar.cookie || {};
  if(!this.options.log) {
    this.options.log = logger({level: logger.NONE});
  }
...
AbstractCouch.prototype.request = function(req, opts, cb, skip) {
  ...
  if((req.headers && !req.headers.cookie)
    ...
    req.headers.cookie = this.jar.cookie[key][user];
  }
  ...
  function callback(err, res, body) {
    ...
    log.info('[%s] %s', res ? res.statusCode : 500, req.url);
